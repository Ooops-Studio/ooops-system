...
```bash
pnpm add @ooopsstudio/events @ooopsstudio/core
```

## Usage

```ts
import {createDevelopmentEvents} from '@ooopsstudio/events/development'

const runtime = await createDevelopmentEvents()
...
await runtime.events.publish('document.created', {id: 'one'})
