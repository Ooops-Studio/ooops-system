...
export function createJobsScheduling(context: JobsKernelContext) {
	return {
		async triggerDueSchedules(): Promise<void> {
			...
			const triggered = await context.withProducerSpan('jobs.schedule.trigger', {triggerTime: now}, async() => context.options.backend.triggerDueSchedules({
				now, maxCatchUp: context.options.maxCatchUp,
			...
			processTriggeredSchedules(context, triggered, context.options.maxCatchUp, generatedRuns)
		}
	}
}
